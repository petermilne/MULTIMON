...
#        self.pvs[re.sub(self.pv_trunc, '', kws['pvname'])] = txt
    ...
    def uut_status_update(self):
        print("uut_status_update {} {} {}".format(self.epics_hn, threading.currentThread().ident, 1))
...
def cas_mon():
    ...
    while True:
        out = casw.stdout.readline().decode('utf-8')
...
def make_hosts(uuts):
    ...
    new_hosts_data = ''.join(hostl)

    if new_hosts_data != hosts_data:
        with open('hosts', 'w') as hf:
            hf.write('# hosts file generated by multimon400 {}\n'.format(str(datetime.date.today())))
            hf.write(new_hosts_data)
        hosts_data = new_hosts_data
